...
x_data <- readxl::read_xlsx(paste0(out_dir,"/","Matris_R_norvegicus.xlsx"))
...
write.table(X_FST,paste0(out_dir,"/","Rattus_norvegicus_data.tsv"),sep = "\t",na = "",row.names = F)
################################################################################
...
tmap_save(filename=paste0(out_dir,"/GRAPHICS/","POINTS_RN_RF",".pdf"),tm=tm_map_points,dpi=900,width =100,height=100,units = "cm");gc()
...
tmap_save(filename=paste0(out_dir,"/GRAPHICS/","POINTS_RN_RF_CLUSTER",".pdf"),tm=tm_map_points2,dpi=900,width =100,height=100,units = "cm");gc()
################################################################################
...
tmap_save(filename=paste0(out_dir,"/GRAPHICS/","Ho_RN",".pdf"),tm=Map3,dpi=900,width =180,height=100,units = "cm");gc()
...
tmap_save(filename=paste0(out_dir,"/GRAPHICS/","Ho_RN_POINTS",".pdf"),tm=Map4,dpi=900,width =180,height=100,units = "cm");gc()


writeRaster(sample_p2_cells3,paste0("D:/DRYAD/RATTUS/",VAR,"_RN",".tif"),overwrite=T)

save.image(paste0("D:/DRYAD/RATTUS/",VAR,"_RN",".RData"))


VAR <- "Ho"
load(paste0("D:/DRYAD/RATTUS/",VAR,"_RN",".RData"))


mess_RN <- dismo::mess(x = layers3[[2:6]],X_FST[,c(4:8)])
writeRaster(mess_RN,paste0("D:/DRYAD/RATTUS/","MESS_RN",".tif"),overwrite=T)

mess_RN2 <- mess_RN

mess_RN2[which(mess_RN2[]<=0)] <- NA
#mess_RR2[which(mess_RR2[]>0)] <- 1

qM <- quantile(mess_RN2,0.5)

mess_RN2[which(mess_RN2[]<=qM)] <- NA
mess_RN2[which(mess_RN2[]>qM)] <- 1
#plot(mess_RR2)

sample_p2_cells3_FILT <- sample_p2_cells3
sample_p2_cells3_FILT <- sample_p2_cells3_FILT*mess_RN2
...
tmap_save(filename=paste0(out_dir,"/GRAPHICS/","Ho_RN_MESS",".pdf"),tm=Map3,dpi=900,width =180,height=100,units = "cm");gc()
...
tmap_save(filename=paste0(out_dir,"/GRAPHICS/","Ho_RN_POINTS_MESS",".pdf"),tm=Map4,dpi=900,width =180,height=100,units = "cm");gc()
